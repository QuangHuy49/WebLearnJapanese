...
import { getGrammarById, updateGrammar } from '../../../../../services/GrammarServices';
...
const EditGrammarPage = () => {
    ...
    const initState = {
        ...
        grammar_note: '',
        grammar_status: ''
    }, [grammar, setGrammar] = useState(initState);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await updateGrammar(id, grammar, csrfToken);
            if (response.status === 201) {
                toast.success('Cập nhật ngữ pháp thành công!');
                navigate(`/admin/lesson/detail-lesson/${grammar.lesson_id}/grammar`);
            } else {
                toast.error('Cập nhật ngữ pháp thất bại. Vui lòng thử lại!');
            }
        } catch (error) {
            console.error('Failed to update grammar:', error);
            toast.error('Đã xảy ra lỗi trong quá trình cập nhật ngữ pháp.');
        }
    };
    ...
    return (
        <div class="flex items-center justify-center p-2">
            <div class="mx-auto w-full bg-white">
                <div class="px-9 pt-4 font-medium text-xl text-custom-color-blue">Cập nhật ngữ pháp</div>
