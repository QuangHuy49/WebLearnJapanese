...
import { addTest } from '../../../services/TestServices';
...
const AddTestPage = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await addTest(formData, csrfToken);
            if (response.status === 201) {
                toast.success('Thêm bài kiểm tra mới thành công!');
                navigate('/admin/test');
            } else {
                toast.error('Thêm bài kiếm tra mới thất bại. Vui lòng thử lại!');
            }
        } catch (error) {
            console.error('Failed to register:', error);
            toast.error('Đã xảy ra lỗi trong quá trình thêm bài kiểm tra.');
        }
    };
    ...
    return (
        <div class="flex items-center justify-center p-2">
            <div class="mx-auto w-full bg-white">
                <div class="px-9 pt-4 font-medium text-xl text-custom-color-blue">Thêm bài kiểm tra mới</div>
