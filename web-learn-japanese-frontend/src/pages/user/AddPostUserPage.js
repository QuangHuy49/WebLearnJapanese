...
import { addPostUser } from '../../services/PostServices';

const AddPostUserPage = () => {
            ...
            uploadTask.on('state_changed',
                ...
                async () => {
                    ...
                    await getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
                        ...
                        setFormData({ ...formData, post_img: downloadURL });
                    });
                }
            );
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await addPostUser(formData, user.user_id, csrfToken);
            if (response.status === 201) {
                toast.success('Thêm bài viết mới thành công. Vui lòng đợi phản hồi từ ADMIN!');
                navigate(`/post/my-post`);
            } else {
                toast.error('Thêm bài viết mới thất bại. Vui lòng thử lại!');
            }
        } catch (error) {
            console.error('Failed to add vocabulary:', error);
            toast.error('Đã xảy ra lỗi trong quá trình thêm bài viết');
        }
    };
    return (
        <div class="flex items-center justify-center p-2">
            <div class="mx-auto w-full bg-white">
                <div class="px-9 pt-4 font-medium text-xl text-custom-color-blue">Thêm bài viết mới</div>
                        ...
                        <div className="mb-5">
                            <div className="rounded-md bg-[#F5F7FB] py-4 px-8">
                                <div className="flex items-center">
                                    {formData.post_img ? (
                                        <>
                                            <img src={formData.post_img} alt="post_img" className="w-[170px] h-[80px] rounded-lg object-cover" />
...
export default AddPostUserPage;